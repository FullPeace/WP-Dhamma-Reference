 ...
 * 
 * This class takes care of the special features of the Book custom post type,
 * by allowing to attachment of files (PDF, MOBI, EPUB) to the post.
 * 
 * @since 0.1.0
 */

class FullPeace_Media_To_Post_Book
{

    /**
     * This is our constructor
     *
     * @return FullPeace_Media_To_Post_Book
     */
    public static function __construct() {

        add_action( 'post_mime_types',         array( __CLASS__, 'book_mime_types'    )    );
        add_action( 'add_meta_boxes',          array( __CLASS__, 'admin_scripts'   ), 5 );
        add_action( 'add_meta_boxes',          array( __CLASS__, 'metabox_add'     )    );
        add_action( 'save_post',               array( __CLASS__, 'book_save_postdata') );
        add_action( 'wp_ajax_refresh_pdf',     array( __CLASS__, 'refresh_pdf' )    );

    }

    public static function book_mime_types() {
        $post_mime_types['application/pdf'] = array( __( 'PDFs' ), __( 'Manage PDFs' ), _n_noop( 'PDF <span class="count">(%s)</span>', 'PDFs <span class="count">(%s)</span>' ) );
        $post_mime_types['application/application/x-mobipocket-book'] = array( __( 'MOBIs' ), __( 'Manage MOBIs' ), _n_noop( 'MOBI <span class="count">(%s)</span>', 'MOBIs <span class="count">(%s)</span>' ) );
    ...
    public static function admin_scripts() {
        ...
        wp_register_script( 'book-metabox-js', plugins_url( '/assets/js/book-metabox.js', __FILE__ ) , array( 'jquery' ), null, true );

    }

    public static function  metabox_add() {
        // Filterable metabox settings.
        $post_types		= array( FullPeace_Media_To_Post::get_slug( 'Books') );
        $context		= 'normal';
        $priority		= 'low';
        ...
        foreach( $post_types as $post_type ) {
            ...
            // Add Metaboxes
            add_meta_box( FullPeace_Media_To_Post::get_slug('book_metaboxes'), __( 'PDF', FPMTP__I18N_NAMESPACE ), array( __CLASS__, 'book_metaboxes' ), $post_type, $context, $priority );
            add_meta_box( FullPeace_Media_To_Post::get_slug('mobi_metabox'), __( 'MOBI', FPMTP__I18N_NAMESPACE ), array( __CLASS__, 'book_metaboxes' ), $post_type, $context, $priority );
            add_meta_box( FullPeace_Media_To_Post::get_slug('epub_metabox'), __( 'EPUB', FPMTP__I18N_NAMESPACE ), array( __CLASS__, 'book_metaboxes' ), $post_type, $context, $priority );
            // Add Necessary Scripts and Styles
            wp_enqueue_media();
            wp_enqueue_script( 'book-metabox-js' );

        }
    }


    public static function book_metaboxes( $post ) {
        $original_post = $post;
        echo self::book_metaboxes_html( $post->ID );
        $post = $original_post;
    }
    ...
    public static function book_metaboxes_html( $post_id ) {

        $post_meta = get_post_custom($post_id);
        $return = '';
        $book_types = array('PDF','EPUB','MOBI');
        foreach ($book_types as $t) 
        {
    ...
    public static  function book_save_postdata($post_id){
